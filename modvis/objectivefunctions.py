...
def check_nan(evaluation, simulation):
    if np.isnan(evaluation).any() or np.isnan(simulation).any():
        raise RuntimeWarning("Nan found in data!")
...
def bias(evaluation, simulation):
    ...
    :rtype: float
    """
    check_nan(evaluation, simulation)
...
def mkge(evaluation, simulation, return_all=False):
    """
    Modified Kling-Gupta Efficiency
    
        mkge = 1 - np.sqrt((cc - 1)**2 + (alpha - 1)**2 + (beta - 1)**2)
    mKGE ranges from -inf to 1 (best model skill), cc ranges from -1 to 1,
    both alpha and beta ranges from 0 to +inf. The closer cc/alpha/beta to 1,
    the higher the mKGE. For example, the correlation measures the temporal dynamics of streamflow while the variability and bias measure the flow duration curve.

    Corresponding paper: 
    Kling, Fuchs, Paulin, 2012, Runoff conditions in the upper Danube basin under an ensemble of climate change scenarios
    output:
        mkge: modified Kling-Gupta Efficiency
    optional_output:
        cc: correlation coefficient
        alpha: ratio of the coefficient of variation (CV) (or relative
        variability)
        beta: ratio of the mean (or bias)
    """
    check_nan(evaluation, simulation)
    if len(evaluation) == len(simulation):
        cc = np.corrcoef(evaluation, simulation)[0, 1]
        alpha = (np.std(simulation) /np.mean(simulation)) / (np.std(evaluation)
                                                             /
                                                             np.mean(evaluation)
                                                             )
